...
Ping the ysi node to see if it's alive.
+ Public Endpoint
...
Returns the current configuation settings of the ysi node.
...
+ Parameters
  + section: ysi-client (string) - configuration section
  + key: server (string) - configuration variable to set
  + value: node.ysi.org (string) - value to set
...
Gets the current state of the registrar. That is, the ysi operations 
...
The ysi core node manages its own wallet -- this has three keys
...
This call instructs the ysi core node to use a particular key
...
`~/.ysi/wallet.json`, and on a subsequent restart, the key will
...
However, for ysi core >= 0.14.5, the `persist_change` keyword
will instruct the core node to write the changed key to
`~/.ysi/wallet.json`. In this mode, the node will backup the
previous wallet to `~/.ysi/wallet.json.prior.<timestamp>`
...
`UPDATE`. `node.ysi.org` will correctly detect the registration
...
+ Response 200 (application/json)
  + Body
               ...
               {
                   "zonefile": "$ORIGIN bar.test\n$TTL 3600\n_https._tcp URI 10 1 \"https://ysi.s3.amazonaws.com/bar.test\"\n"
               }
...
+ Response 200 (application/json)
  + Body
              ...
              "status": "registered", 
              "zonefile": "$ORIGIN muneeb.id\n$TTL 3600\n_http._tcp IN URI 10 1 \"https://ysi.s3.amazonaws.com/muneeb.id\"\n", 
...
The Gaia endpoints interface with `ysi-storage.js` to provide
storage to ysi applications.
                 ...
                 "fred.id": {
                   ...
                   "profile": {
                     ...
                     "image": [
                       {
                         "@type": "ImageObject", 
                         "contentUrl": "https://gaia.ysi.org/hub/1CER5u4QXuqffHjHKrU76iMCsqtJLM5VHu/0/avatar-0", 
                         "name": "avatar"
                       }
                     ], 
                     "name": "Fred Wilson"
                     ...
                     "uri": [
                       {
                         "name": "_http._tcp", 
                         "priority": 10, 
                         "target": "https://gaia.ysi.org/hub/1CER5u4QXuqffHjHKrU76iMCsqtJLM5VHu/0/profile.json", 
                         "weight": 1
                       }
                     ]
                   }
