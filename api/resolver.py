...
from ysi_proofs import profile_to_proofs, profile_v3_to_proofs

import ysi_client.profile
import ysi_client.subdomains

from ysi_client.schemas import OP_NAME_PATTERN, OP_NAMESPACE_PATTERN
...
def get_profile(fqa):
    ...
    if not is_valid_fqa(fqa):
        fqa = str(fqa)
        res = ysi_client.subdomains.is_address_subdomain(fqa)
        if res:
            subdomain, domain = res[1]
            try:
                resp = ysi_client.subdomains.resolve_subdomain(subdomain, domain)
                data = { 'profile' : resp['profile'],
                         'zone_file': resp['zonefile'],
                         'public_key': resp.get('public_key', None),
                         'verifications' : [] }
                return data
            except ysi_client.subdomains.SubdomainNotFound as e:
                log.exception(e)
    ...
    try:
        res = ysi_client.profile.get_profile(
