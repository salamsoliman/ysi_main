...
def check( state_engine, nameop, block_id, checked_ops ):
    ...
    if EPOCH_FEATURE_NAMESPACE_BURN_TO_CREATOR in epoch_features:
        ...
        if namespace_version_bits != NAMESPACE_VERSION_PAY_TO_BURN:
           log.debug("Namespace '%s' requires version %s, but this ysi is version %s" % (namespace_id, nameop['version'], NAMESPACE_VERSION_PAY_TO_BURN))
           return False
...
def snv_consensus_extras( name_rec, block_id, blockchain_name_data, db ):
    ...
    """
    return ysi_client.operations.namespacereveal.snv_consensus_extras( name_rec, block_id, blockchain_name_data )
    '''
    return {}
    '''
