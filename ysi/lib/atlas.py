...
from ysi_client.config import semver_newer
from ysi_client.utils import url_to_host_port, atlas_inventory_to_string

from ysi_client.proxy import \
        ping as ysi_ping, \
        getinfo as ysi_getinfo, \
        get_zonefile_inventory as ysi_get_zonefile_inventory, \
        get_atlas_peers as ysi_get_atlas_peers, \
        get_zonefiles as ysi_get_zonefiles, \
        put_zonefiles as ysi_put_zonefiles
...
if os.environ.get("BLOCKSTACK_TEST", None) == "1" and os.environ.get("BLOCKSTACK_ATLAS_NETWORK_SIMULATION", None) == "1" and os.environ.get("BLOCKSTACK_ATLAS_NETWORK_SIMULATION_PEER", None) == "1":
    ...
    # production
    from ysi_client import BlockstackRPCClient

def time_now():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests.atlas_network import time_now
        return time_now()
...
def time_sleep(hostport, procname, value):
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import time_sleep as method
...
def atlas_max_neighbors():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_max_neighbors as method
        return method()
...
def atlas_peer_lifetime_interval():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_peer_lifetime_interval as method
        return method()
...
def atlas_peer_ping_interval():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_peer_ping_interval as method
        return method()
...
def atlas_peer_max_age():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_peer_max_age as method
        return method()
...
def atlas_peer_clean_interval():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_peer_clean_interval as method
        return method()
...
def atlas_ping_timeout():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_ping_timeout as method
        return method()
...
def atlas_inv_timeout():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_inv_timeout as method
        return method()
...
def atlas_neighbors_timeout():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_neighbors_timeout as method
        return method()
...
def atlas_zonefiles_timeout():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_zonefiles_timeout as method
        return method()
...
def atlas_push_zonefiles_timeout():
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import atlas_push_zonefiles_timeout as method
        return method()
...
def get_rpc_client_class():
    ...
    global ATLAS_TEST
    if ATLAS_TEST:
        from ysi_integration_tests import AtlasRPCTestClient
...
def atlas_peer_get_zonefile_inventory_range( my_hostport, peer_hostport, bit_offset, bit_count, timeout=None, peer_table=None ):
    ...
    try:
        zf_inv = ysi_get_zonefile_inventory( peer_hostport, bit_offset, bit_count, timeout=timeout, my_hostport=my_hostport, proxy=rpc )
...
def atlas_peer_get_neighbors( my_hostport, peer_hostport, timeout=None, peer_table=None, con=None, path=None ):
    ...
    try:
        peer_list = ysi_get_atlas_peers( peer_hostport, timeout=timeout, my_hostport=my_hostport, proxy=rpc )
...
def atlas_get_zonefiles( my_hostport, peer_hostport, zonefile_hashes, timeout=None, peer_table=None ):
    ...
    for zf_batch in zf_batches:
        zf_payload = None
        try:
            zf_payload = ysi_get_zonefiles( peer_hostport, zf_batch, timeout=timeout, my_hostport=my_hostport, proxy=rpc )
...
def atlas_zonefile_push( my_hostport, peer_hostport, zonefile_data, timeout=None, peer_table=None ):
    ...
    zonefile_hash = ysi_client.get_zonefile_data_hash(zonefile_data)
    ...
    try:
        push_info = ysi_put_zonefiles( peer_hostport, [zonefile_data_b64], timeout=timeout, my_hostport=my_hostport, proxy=rpc )
