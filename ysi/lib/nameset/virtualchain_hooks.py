...
import virtualchain
log = virtualchain.get_logger("ysi-log")
...
def db_parse( block_id, txid, vtxindex, op, data, senders, inputs, outputs, fee, db_state=None ):
   ...
   Parse a ysi operation from a transaction's nulldata (data) and a list of outputs, as well as 
...
def db_check( block_id, new_ops, op, op_data, txid, vtxindex, checked_ops, db_state=None ):
    ...
    Is this operation consistent with ysi's rules?
