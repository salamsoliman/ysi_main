...
ysi_db = None
ysi_db_lastblock = None
ysi_db_lock = threading.Lock()
...
class BlockstackDB( virtualchain.StateEngine ):
    """
    State engine implementation for ysi.
    """
    ...
    def __init__( self, db_filename, disposition, expected_snapshots={} ):
        ...
        lastblock = self.get_lastblock( impl=ysi_impl )
        ...
        # announcers to track
        ysi_opts = default_ysi_opts( virtualchain.get_config_filename(impl=ysi_impl), virtualchain_impl=ysi_impl )
        self.announce_ids = ysi_opts['announcers'].split(",")

        self.set_backup_frequency( ysi_opts['backup_frequency'] )
        self.set_backup_max_age( ysi_opts['backup_max_age'] )
    ...
    def borrow_readwrite_instance( cls, db_path, block_number, expected_snapshots={} ):
        ...
        global ysi_db, ysi_db_lastblock, ysi_db_lock

        ysi_db_lock.acquire()

        try:
            assert ysi_db is None, "Borrowing violation"
        except Exception, e:
            log.exception(e)
            log.error("FATAL: Borrowing violation")
            os.abort()

        ysi_db = BlockstackDB( db_path, DISPOSITION_RW, expected_snapshots=expected_snapshots )
        ysi_db_lastblock = block_number
        ysi_db_lock.release()
    ...
    def release_readwrite_instance( cls, db_inst, block_number ):
        ...
        try:
            assert ysi_db is not None, "Borrowing return violation: db is None"
            assert ysi_db == db_inst, "Borrowing return violation: different instances"
            assert ysi_db_lastblock == block_number, "Borrowing return violation: different blocks"
        except Exception, e:
            log.exception(e)
    ...
    def export_db( self, path ):
        ...
        should just pull a backup db from ~/.ysi-server/backups
    ...
    def check_preorder_collision( self, preorder_hash, block_id, checked_ops ):
        ...
        Not called directly; called by the @state_preorder() decorator in ysi.lib.operations.preorder and ysi.lib.operations.namespacepreorder
        """
    ...
    def check_name_collision( self, name, block_id, checked_ops ):
        ...
        Not called directly; called by the @state_create() decorator in ysi.lib.operations.register
        """
    ...
    def check_namespace_collision( self, namespace_id, block_id, checked_ops ):
        ...
        Not called directly; called by the @state_create() decorator in ysi.lib.operations.namespacereveal
        """
