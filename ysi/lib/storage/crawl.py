...
import ysi_client
from ysi_client import get_zonefile_data_hash, verify_zonefile
...
def get_zonefile_data_from_storage( name, zonefile_hash, drivers=None ):
    ...
    zonefile_txt = ysi_client.storage.get_immutable_data( zonefile_hash, hash_func=ysi_client.get_blockchain_compat_hash, fqu=name, zonefile=True, drivers=drivers )
...
def store_zonefile_data_to_storage( zonefile_text, txid, required=None, skip=None, cache=False, zonefile_dir=None, tx_required=True ):
    ...
    res = ysi_client.storage.put_immutable_data( zonefile_text, txid, data_hash=zonefile_hash, required=required, skip=skip, required_exclusive=True )
    if res is None:
...
def store_mutable_data_to_storage( blockchain_id, data_id, data_txt, profile=False, required=None, skip=None ):
    ...
    res = ysi_client.storage.put_mutable_data(nocollide_data_id, data_txt, sign=False, raw=True, required=required, skip=skip, blockchain_id=blockchain_id)
    return res
...
def load_mutable_data_from_storage( blockchain_id, data_id, drivers=None ):
    ...
    nocollide_data_id = '{}-{}'.format(blockchain_id, data_id)
    res = ysi_client.storage.get_mutable_data(nocollide_data_id, None, blockchain_id=blockchain_id, drivers=drivers, decode=False)
    return res
