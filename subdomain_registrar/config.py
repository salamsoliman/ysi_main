...
from ysi_client import config as ysi_client_config
...
def __get_or_load_config():
    ...
    filename = os.environ.get(
        ...
        os.path.expanduser("~/.ysi_subdomains/config.ini"))
...
def get_core_auth():
    ...
    DEFAULT_CONFIG_FILE = os.environ.get(
        "BLOCKSTACK_CLIENT_CONFIG", "~/.ysi/client.ini")
    config_file = os.path.expanduser(c.get("core_config", DEFAULT_CONFIG_FILE)) 
    auth = ysi_client_config.get_config(config_file)['api_password']
    assert auth
    return auth
