...
from ysi_client.constants import FIRST_BLOCK_MAINNET
from ysi_client.logger import get_logger
from ysi_client.utils import url_to_host_port, atlas_inventory_to_string

from ysi_client import BlockstackRPCClient

from ysi import RPC_SERVER_PORT
# from ysi.atlas import *
...
def atlas_network_get_peer_conf( network_dir, portnum ):
    ...
    conf = ysi.config.configure( config_file=conf_path, interactive=False )
    return conf
...
class MockDB(object):
    ...
    def add_zonefile( self, zf ):
        ...
        """
        zfhash = ysi_client.get_zonefile_data_hash( str(zf) )
    ...
    def mock_make_zonefile( cls, id_idx ):
        ...
        """
        new_zf = ysi_client.user.make_empty_user_zonefile( "testregistration%03d.id" % (id_idx),  
                    ...
                    ["http://node.ysi.org:6264/RPC2#testregistration%03d.id" % (id_idx),
                     "file:///home/test/.ysi/storage-disk/mutable/testregistration%03d.id" % (id_idx)] )
    ...
    def mock_add_zonefile_hashes(self, count, present=True):
        ...
        for i in xrange(0, count):
            ...
            new_zf_hash = ysi_client.storage.hash_zonefile( new_zf )
            ...
            if new_zf is not None:
                ...
                ysi.lib.storage.store_cached_zonefile( new_zf, zonefile_dir=self.zonefile_dir )
    ...
    def mock_append_zonefile( self, zfhash, zf=None, same_block=False ):
        ...
        if zf is not None:
            assert ysi_client.storage.hash_zonefile(zf) == zfhash
...
class AtlasNode(object):
    ...
    def __init__(self, host, port, peer_seeds, peer_blacklist, zonefile_inv, db_path, zonefile_dir, no_db=False):
        ...
        from ysi import AtlasPeerCrawler, AtlasHealthChecker, AtlasZonefileCrawler
    ...
    def append_zonefile(self, zfhash, zf=None ):
        ...
        if zf is not None:
            ...
            ysi.lib.storage.store_cached_zonefile( zf, zonefile_dir=self.zonefile_dir )
...
class AtlasStaticNetwork( SocketServer.ThreadingMixin, SimpleXMLRPCServer ):
    ...
    def step(self):
        ...
        if hostport is not None:
            ...
            for node_hostport in self.node_hosts.keys():
                ...
                zfhash = ysi_client.storage.hash_zonefile(new_zf)
        ...
        for zfh in zonefile_hashes:
            if db.get_zonefile( zfh ) is None:
                ...
                if zf is None:
                    ...
                    ret.append( {zfh: ysi_zones.make_zone_file(zf)} )

            else:
                ret.append({zfh: ysi_zones.make_zone_file(db.get_zonefile(zfh))})
    ...
    def rpc_put_zonefiles( self, src_hostport, dest_hostport, zonefile_info ):
        ...
        for zfdata in zonefile_info:
            zf = ysi_zones.parse_zone_file( str(zfdata) )
            zfhash = ysi_client.get_zonefile_data_hash( str(zfdata) )
...
if __name__ == "__main__":
    ...
    os.makedirs(zonefile_dir)
    ysi_client.session( config_path=sys.argv[1] )

    virtualchain.setup_virtualchain( impl=ysi.lib.virtualchain_hooks )
...
if __name__ == "__main__":
    ...
    def test_atlas_make_zonefile_inventory( db, path ):
        ...
        assert inv_vec == ysi.atlas.ZONEFILE_INV, "Inv mismatch: %s != %s" % (binascii.hexlify(inv_vec), binascii.hexlify(ysi.atlas.ZONEFILE_INV))
    ...
    test_peer_seeds = ['node.ysi.org:6264']
