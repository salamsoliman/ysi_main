...
def scenario( wallets, **kw ):
    ...
    res = testlib.ysi_cli_app_signin("foo.test", datastore_pk, 'register.app', ['names', 'register', 'prices', 'revoke', 'zonefiles', 'blockchain', 'node_read'])
    if 'error' in res:
        print json.dumps(res, indent=4, sort_keys=True)
        error = True
        return 
    ...
    # present?
    res = testlib.ysi_REST_call('GET', '/v1/names?page=0', ses )
    if 'error' in res or res['http_status'] != 200:
    ...
    # revoke it
    res = testlib.ysi_REST_call("DELETE", "/v1/names/bar.test", ses )
