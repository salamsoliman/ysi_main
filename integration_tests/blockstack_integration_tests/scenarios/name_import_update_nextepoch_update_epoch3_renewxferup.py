...
TEST ENV BLOCKSTACK_EPOCH_2_END_BLOCK 694
...
def scenario( wallets, **kw ):
    ...
    if whois['expire_block'] != testlib.get_current_block(**kw) + 2:
        print 'wrong expire block (now: {})'.format(testlib.get_current_block(**kw))
        print whois
        return False
    ...
    if whois['expire_block'] != testlib.get_current_block(**kw) + 3:
        print 'wrong expire block (should be 3 blocks away from {})'.format(testlib.get_current_block(**kw))
        print whois
        return False

    print whois

    resp = testlib.ysi_name_update( 'foo.test', '33' * 20, wallets[3].privkey )
    if 'error' in resp:
        print resp
        return False
    ...
    resp = testlib.ysi_name_update( 'foo.test', '44' * 20, wallets[3].privkey )
    if 'error' in resp:
        print resp
        return False
    ...
    resp = testlib.ysi_name_update( 'foo.test', '66' * 20, wallets[3].privkey )
...
def check( state_engine ):
    ...
    # renewed (6 blocks later, starting from renewal time)
    if name_rec['last_renewed'] - 6 != name_rec['first_registered']:
