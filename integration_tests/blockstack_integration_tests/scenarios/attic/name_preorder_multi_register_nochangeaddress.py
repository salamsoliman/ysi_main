...
def scenario( wallets, **kw ):
    ...
    resp = testlib.ysi_name_register( "foo.test", wallets[2].privkey, wallets[5].addr )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        sys.exit(1)
        
    resp = testlib.ysi_name_register( "bar.test", wallets[2].privkey, wallets[3].addr )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        sys.exit(1)

    resp = testlib.ysi_name_register( "baz.test", wallets[2].privkey, wallets[4].addr )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        sys.exit(1)
...
def check( state_engine ):
    ...
    if preorder['op_fee'] < ysid.get_name_cost( 'foo.test' ) + ysid.get_name_cost( 'bar.test' ) + ysid.get_name_cost( 'baz.test' ):
