...
log = virtualchain.get_logger("name_preorder_register_update_ysiurl_nodatakey")
...
def scenario( wallets, **kw ):
    ...
    rc = ysi_client.storage.put_immutable_data( None, result_1['transaction_hash'], data_hash=legacy_hash, data_text=legacy_txt )
    assert rc is not None

    rc = ysi_client.storage.put_immutable_data( None, result_2['transaction_hash'], data_hash=legacy_hash, data_text=legacy_txt )
    assert rc is not None
    ...
    testlib.ysi_client_set_wallet( "0123456789abcdef", wallet_keys['payment_privkey'], wallet_keys['owner_privkey'], None )
    ...
    testlib.ysi_client_set_wallet( "0123456789abcdef", wallet_keys_2['payment_privkey'], wallet_keys_2['owner_privkey'], None )
    log.debug("put mutable for bar.test")
    put_result = testlib.ysi_cli_put_mutable( 'bar.test', 'hello_world_mutable', json.dumps({'hello': 'world'}), password='0123456789abcdef' )
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True )
        return False
    ...
    testlib.ysi_client_set_wallet( "0123456789abcdef", wallet_keys['payment_privkey'], wallet_keys['owner_privkey'], None )
    res = testlib.ysi_cli_put_mutable( 'foo.test', 'foo_data2', json.dumps({'hello2': 'world2'}), password='0123456789abcdef')
    if 'error' in res:
        print json.dumps(res, indent=4, sort_keys=True)
        return False
    ...
    res = testlib.ysi_cli_put_immutable('foo.test', 'foo_immutable', json.dumps({'hello3': 'world3'}), password='0123456789abcdef')
    if 'error' in res:
        print json.dumps(res, indent=4, sort_keys=True)
        return False
    ...
    datastore_id_res = testlib.ysi_cli_datastore_get_id( "foo-app.com" )
    datastore_name = datastore_id_res['datastore_id']

    # make datastore 
    res = testlib.ysi_cli_create_datastore( "foo-app.com" )
    if 'error' in res:
    ...
    log.debug("putfile")
    res = testlib.ysi_cli_datastore_putfile( "foo-app.com", '/hello_datastore', data )
    if 'error' in res:
    ...
    # make a directory 
    log.debug("mkdir")
    res = testlib.ysi_cli_datastore_mkdir( "foo-app.com", '/hello_dir' )
    if 'error' in res:
    ...
    res = testlib.ysi_cli_datastore_putfile( "foo-app.com", '/hello_dir/hello_dir_datastore', data )
    if 'error' in res:
...
def get_data( url ):
    ...
    Test urllib2 opener
    """
    handler = ysi_client.BlockstackHandler(full_response=True)
    opener = urllib2.build_opener( handler )
    dat = opener.open( url )
    res1 = json.loads( dat.read() )

    res2 = testlib.ysi_cli_get_data(url)
    if 'error' in res2:
        return res2
...
def check( state_engine ):
    ...
    for i in xrange(0, len(names)):
        ...
        user_zonefile = ysi_client.load_name_zonefile( name, zonefile_hash )
        ...
        user_profile = ysi_client.get_profile(name, user_zonefile=user_zonefile)
    ...
    immutable_data = get_data( "ysi://hello_world_immutable.foo.test/" )
    ...
    immutable_data = get_data( "ysi://hello_world_immutable.foo.test/#%s" % immutable_hash )
    if 'error' in immutable_data:
        print json.dumps(immutable_data, indent=4, sort_keys=True)
        return False 
    ...
    try:
        immutable_data = get_data( "ysi://hello_world_immutable.foo.test/#%s" % ("0" * len(immutable_hash)))
        print "no error"
        print json.dumps(immutable_data, indent=4, sort_keys=True)
        return False
    ...
    immutable_data_list = get_data( "ysi://foo.test/#immutable" )
    ...
    device_id = ysi_client.config.get_local_device_id()
    data_id = ysi_client.storage.make_fq_data_id( device_id, 'hello_world_mutable' )

    # can fetch latest mutable by name
    mutable_data = get_data( "ysi://bar.test/{}".format(data_id))
    ...
    mutable_data = get_data( "ysi://bar.test/{}#1".format(data_id))
    if 'error' in mutable_data:
        print json.dumps(mutable_data, indent=4, sort_keys=True)
        return False 
    ...
    try:
        mutable_data = get_data("ysi://bar.test/{}#2".format(data_id))
    ...
    data_id = ysi_client.storage.make_fq_data_id(device_id, 'foo_data2')
    mutable_data = get_data( "ysi://foo.test/{}".format(data_id) )
    ...
    if json.loads(mutable_data['data']) != {'hello2': 'world2'}:
        print "Invalid mutable data fetched from ysi://foo.test/{}".format(data_id)
        print json.dumps(mutable_data, indent=4, sort_keys=True)
        return False

    # can fetch immutable data put by URL 
    immutable_data = get_data( "ysi://foo_immutable.foo.test" )
    if 'error' in immutable_data or 'hash' not in immutable_data:
        print json.dumps(immutable_data, indent=4, sort_keys=True)
        return False

    if json.loads(immutable_data['data']) != {'hello3': 'world3'}:
        print "Invalid immutable data fetched from ysi://foo_immutable.foo.test" 
        print json.dumps(immutable_data, indent=4, sort_keys=True)
        return False

    # can fetch files and directories
    mutable_data = get_data( "ysi://{}@foo-app.com/hello_datastore".format(datastore_name) )
    if 'error' in mutable_data or 'file' not in mutable_data or mutable_data['file']['idata'] != 'hello datastore':
        print 'Failed to get ysi://{}@foo-app.com/hello_datastore'.format(datastore_name)
        print json.dumps(mutable_data, indent=4, sort_keys=True)
        return False

    mutable_data = get_data( "ysi://{}@foo-app.com/hello_dir/".format(datastore_name) )
    if 'error' in mutable_data or 'dir' not in mutable_data or 'hello_dir_datastore' not in mutable_data['dir']['idata'].keys():
        print 'Failed to get ysi://{}@foo-app.com/hello_dir/'.format(datastore_name)
        print json.dumps(mutable_data, indent=4, sort_keys=True)
        return False

    mutable_data = get_data( "ysi://{}@foo-app.com/hello_dir/hello_dir_datastore".format(datastore_name) )
    if 'error' in mutable_data or 'file' not in mutable_data or mutable_data['file']['idata'] != 'hello dir datastore':
        print 'Failed to get ysi://{}@foo-app.com/hello_dir/hello_dir_datastore'.format(datastore_name)
        print json.dumps(mutable_data, indent=4, sort_keys=True)
        return False
