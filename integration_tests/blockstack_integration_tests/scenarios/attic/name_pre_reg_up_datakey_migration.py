...
log = ysi_client.get_logger("ysi-integration-tests")
...
def scenario( wallets, **kw ):
    ...
    wallet_keys = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[2].privkey, wallets[3].privkey, None )
    ...
    put_result = testlib.ysi_cli_put_immutable("foo.test", "hello_world_1_immutable", json.dumps(datasets[0], sort_keys=True), password='0123456789abcdef')
    ...
    testlib.ysi_cli_put_immutable("foo.test", "hello_world_2_immutable", json.dumps(datasets[1], sort_keys=True), password='0123456789abcdef')
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
    ...
    put_result = testlib.ysi_cli_put_mutable("foo.test", "hello_world_1_mutable", json.dumps(datasets[0], sort_keys=True), password='0123456789abcdef')
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
    ...
    put_result = testlib.ysi_cli_put_mutable("foo.test", "hello_world_2_mutable", json.dumps(datasets[1], sort_keys=True), password='0123456789abcdef')
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
    ...
    res = ysi_client.set_data_pubkey("foo.test", wallets[4].pubkey_hex, wallet_keys=wallet_keys, proxy=test_proxy )
    if 'error' in res:
        print json.dumps(res, indent=4, sort_keys=True)
        return False 
    ...
    # restart daemon
    wallet_keys = testlib.ysi_client_set_wallet( "0123456789abcdef", wallet_keys['payment_privkey'], wallet_keys['owner_privkey'], wallet_keys['data_privkey'] ) 
    ...
    # put immutable
    put_result = testlib.ysi_cli_put_immutable("foo.test", "hello_world_3_immutable", json.dumps(datasets[2], sort_keys=True), password='0123456789abcdef')
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
    ...
    put_result = testlib.ysi_cli_put_mutable("foo.test", "hello_world_3_mutable", json.dumps(datasets[2], sort_keys=True), password='0123456789abcdef')
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
    ...
    result = testlib.ysi_cli_delete_immutable("foo.test", "hello_world_1_immutable", password='0123456789abcedef')
    if 'error' in result:
    ...
    result = testlib.ysi_cli_delete_mutable("foo.test", "hello_world_1_mutable", password='0123456789abcdef')
    if 'error' in result:
        print json.dumps(result, indent=4, sort_keys=True)
        return False
...
def check( state_engine ):
    ...
    for i in xrange(1, len(datasets)):
        immutable_data = testlib.ysi_cli_get_immutable('foo.test', 'hello_world_{}_immutable'.format(i+1))
        ...
        mutable_data = testlib.ysi_cli_get_mutable('foo.test', 'hello_world_{}_mutable'.format(i+1))
    ...
    # should fail 
    immutable_data = testlib.ysi_cli_get_immutable('foo.test', 'hello_world_1_immutable')
    if immutable_data is not None and 'error' not in immutable_data:
        print "Got deleted immutable dataset"
        return False

    mutable_data = testlib.ysi_cli_get_mutable( "foo.test", "hello_world_1_mutable")
