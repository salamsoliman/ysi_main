...
def scenario( wallets, **kw ):
    ...
    resp = testlib.ysi_name_preorder_multi( names, wallets[2].privkey, [wallets[5].addr, wallets[6].addr, wallets[7].addr])
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        sys.exit(1)
    ...
    for i in xrange(0, len(names)):
        ...
        resp = testlib.ysi_name_register( name, wallets[2].privkey, register_wallet.addr )
        if debug or  'error' in resp:
            print json.dumps( resp, indent=4 )
