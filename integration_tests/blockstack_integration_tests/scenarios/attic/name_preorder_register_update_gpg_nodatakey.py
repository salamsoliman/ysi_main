...
def scenario( wallets, **kw ):
    ...
    wallet_keys = ysi_client.make_wallet_keys( owner_privkey=wallets[3].privkey, payment_privkey=wallets[8].privkey )
    wallet_keys_2 = ysi_client.make_wallet_keys( owner_privkey=wallets[6].privkey, payment_privkey=wallets[9].privkey )
...
def check( state_engine ):
    ...
    for i in xrange(0, len(names)):
        ...
        account_key_listing = ysi_gpg.gpg_list_profile_keys( name )
        secure_app_listing = ysi_gpg.gpg_list_app_keys( name, "secure_messaging" )
        less_secure_app_listing = ysi_gpg.gpg_list_app_keys( name, "less-secure_messaging" )
        ...
        profile_key = ysi_gpg.gpg_profile_get_key( name, account_key_listing[0]['keyName'], gpghome=gpghome )
        if 'error' in profile_key:
            print "no key in account %s: %s" % (account_key_listing, profile_key['error'])
            return False

        secure_app_key = ysi_gpg.gpg_app_get_key( name, "secure_messaging", secure_app_listing[0]['keyName'], \
                                                         immutable=True )
        ...
        less_secure_app_key = ysi_gpg.gpg_app_get_key( name, "less-secure_messaging", less_secure_app_listing[0]['keyName'] )
