...
def scenario( wallets, **kw ):
    ...
    result = testlib.ysi_name_update( "foo.test", legacy_hash, wallets[3].privkey )
    ...
    rc = ysi_client.storage.put_immutable_data( legacy_txt, result['transaction_hash'], data_hash=legacy_hash )
    ...
    put_result = ysi_client.put_immutable( "foo.test", "hello_world_1", json.dumps(datasets[0], sort_keys=True), proxy=test_proxy, wallet_keys=wallet_keys )
    ...
    put_result = ysi_client.put_immutable( "foo.test", "hello_world_2", json.dumps(datasets[1], sort_keys=True), proxy=test_proxy, wallet_keys=wallet_keys )
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
    ...
    put_result = ysi_client.put_immutable( "foo.test", "hello_world_3", json.dumps(datasets[2], sort_keys=True), proxy=test_proxy, wallet_keys=wallet_keys )
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
    ...
    put_result = ysi_client.put_immutable( "foo.test", "hello_world_3", json.dumps(datasets[0], sort_keys=True), proxy=test_proxy, wallet_keys=wallet_keys )
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True )
...
def check( state_engine ):
    ...
    for i in xrange(0, len(hist)):
        test_hist = ysi_client.list_immutable_data_history( "foo.test", "hello_world_%s" % (i+1))
        if test_hist != hist[i]:
