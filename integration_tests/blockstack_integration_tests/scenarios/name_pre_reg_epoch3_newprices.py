...
def scenario( wallets, **kw ):
    ...
    for n in names + names_novowel + names_nonalpha:
        if new_name_costs[n] * 10 != name_costs[n]:
            print 'wrong name cost change for {}: {} * 10 != {}'.format(n, new_name_costs[n], name_costs[n])
            return False

    for ns in namespaces:
        if new_namespace_costs[ns] * 10 != namespace_costs[ns]:
            print 'wrong namespace cost change for {}: {} * 10 != {}'.format(ns, new_name_costs[ns], name_costs[ns])
            return False
...
def check( state_engine ):
    ...
    # renewed 
    if name_rec['last_renewed'] != 695:
        print 'wrong last_renewed'
        print name_rec
        return False
