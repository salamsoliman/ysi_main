...
import keylib
import requests
from ysi_integration_tests.scenarios import testlib
import atexit
...
""".format(ysi_client.config.read_config_file()['ysi-client']['api_endpoint_port'])
    ...
    uri_rec = ysi_client.zonefile.url_to_uri_record("file:///tmp/baz.profile.json")
...
def check( state_engine ):
    subdomain = "bar"
    domain = "foo.id"
    user_profile = ysi_client.subdomains.resolve_subdomain(subdomain, domain)['profile']
