...
def scenario( wallets, **kw ):
    ...
    # wait for a bit...
    for i in xrange(0, 6 * NAMESPACE_LIFETIME_MULTIPLIER - 1):
        testlib.next_block( **kw )
...
def check( state_engine ):
    ...
    # renewed (6 * lifetime_multiplier + 1 blocks later, starting from renewal time)
    if name_rec['last_renewed'] - (6 * NAMESPACE_LIFETIME_MULTIPLIER + 1) != name_rec['first_registered']:
