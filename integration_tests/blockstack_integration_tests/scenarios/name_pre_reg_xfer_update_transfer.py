...
def scenario( wallets, **kw ):
    ...
    # now update (5)
    resp = testlib.ysi_name_update( "foo.test", "22" * 20, wallets[5].privkey)
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )

    testlib.next_block( **kw )

    resp = testlib.ysi_name_transfer( "foo.test", wallets[4].addr, True, wallets[5].privkey)
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )

    testlib.next_block( **kw )

    resp = testlib.ysi_name_update( "foo.test", "33" * 20, wallets[4].privkey)
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )

    testlib.next_block( **kw )

    resp = testlib.ysi_name_update( "foo.test", "44" * 20, wallets[4].privkey)
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )

    testlib.next_block( **kw )

    resp = testlib.ysi_name_transfer( "foo.test", wallets[5].addr, True, wallets[4].privkey)
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )
    ...
    resp = testlib.ysi_name_update( "foo.test", "55" * 20, wallets[4].privkey)
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )
    ...
    resp = testlib.ysi_name_update( "foo.test", "11" * 20, wallets[5].privkey)
    if debug or 'error' in resp:
        print json.dumps( resp, indent=4 )
