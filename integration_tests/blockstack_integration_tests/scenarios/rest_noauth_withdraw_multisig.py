...
def scenario( wallets, **kw ):
    ...
    wallet_keys = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[5].privkey, wallets[3].privkey, wallets[0].privkey )
    ...
    testlib.ysi_name_preorder( "foo.test", wallets[2].privkey, wallets[3].addr, wallet=wallets[3])
    testlib.next_block( **kw )
    
    testlib.ysi_name_register( "foo.test", wallets[2].privkey, wallets[3].addr, wallet=wallets[3] )
    testlib.next_block( **kw )
    ...
    res = testlib.ysi_REST_call('GET', '/v1/blockchains/bitcoin/pending', None, api_pass=api_pass )
    if 'error' in res:
    ...
    # what's the balance?
    res = testlib.ysi_REST_call('GET', '/v1/wallet/balance', None, api_pass=api_pass )
    ...
    res = testlib.ysi_REST_call('POST', '/v1/wallet/balance', None, api_pass=api_pass, data={'message': 'hello multisig!', 'address': wallets[3].addr} )
    if res['http_status'] != 200:
