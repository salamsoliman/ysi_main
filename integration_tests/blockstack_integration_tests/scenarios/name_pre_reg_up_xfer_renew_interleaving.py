...
def do_interleaving( name, namerecs, order ):
    ...
    for i in xrange(0, len(order)):
        ...
        if op == 'r':
            ...
            resp = testlib.ysi_name_renew( name, namerecs[name][0].privkey, safety_checks=False, tx_fee=10000*5 )
            if 'error' in resp:
                print json.dumps( resp, indent=4 )

        elif op == 'u':
            # update
            resp = testlib.ysi_name_update( name, ("%s%s" % (i, i)) * 20, namerecs[name][0].privkey, safety_checks=False )
            if 'error' in resp:
                print json.dumps( resp, indent=4 )
            ...
            resp = testlib.ysi_name_transfer( name, namerecs[name][1].addr, True, namerecs[name][0].privkey, safety_checks=False )
            if 'error' in resp:
                print json.dumps( resp, indent=4 )
...
def scenario( wallets, **kw ):
    ...
    for name in names.keys():
        ...
        resp = testlib.ysi_name_preorder( name, names[name][1].privkey, names[name][0].addr )
        if debug or  'error' in resp:
            print json.dumps( resp, indent=4 )
    ...
    for name in names.keys():
        ...
        resp = testlib.ysi_name_register( name, names[name][1].privkey, names[name][0].addr )
        if debug or  'error' in resp:
            print json.dumps( resp, indent=4 )
