...
def scenario( wallets, **kw ):
    ...
    wallet = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[5].privkey, wallets[3].privkey, wallets[4].privkey )
    ...
    put_result = testlib.ysi_cli_put_immutable( "foo.test", "hello_world_1", json.dumps(datasets[0], sort_keys=True), password="0123456789abcdef" )
    ...
    put_result = testlib.ysi_cli_put_immutable( "foo.test", "hello_world_2", json.dumps(datasets[1], sort_keys=True), password='0123456789abcdef')
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
    ...
    put_result = testlib.ysi_cli_put_immutable( "foo.test", "hello_world_3", json.dumps(datasets[2], sort_keys=True), password='0123456789abcdef')
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
...
def check( state_engine ):
    ...
    for i in xrange(0, len(datasets)):
        immutable_data = testlib.ysi_cli_get_immutable( "foo.test", immutable_data_hashes[i] )
        ...
        immutable_data_by_name = testlib.ysi_cli_get_immutable("foo.test", "hello_world_%s" % (i+1))
