...
def scenario( wallets, **kw ):
    ...
    resp = testlib.ysi_name_transfer( 'foo.test', wallets[0].addr, True, wallets[2].privkey)
    if 'error' in resp:
        print resp
        return False
    ...
    resp = testlib.ysi_name_register("foo.test", wallets[3].privkey, wallets[4].addr, zonefile_hash='33' * 20)
    if 'error' in resp:
        print resp
        return False
