...
def scenario( wallets, **kw ):
    ...
    resp = testlib.ysi_name_import( "foo.test", wallets[9].addr, "11" * 20, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
    ...
    resp = testlib.ysi_name_transfer( "foo.test", wallets[10].addr, True, wallets[9].privkey)
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
...
def check( state_engine ):
    ...
    # transferred 
    if name_rec['address'] != wallets[10].addr or name_rec['sender'] != virtualchain.make_payment_script( wallets[10].addr ):
        print "wrong owner"
        return False
