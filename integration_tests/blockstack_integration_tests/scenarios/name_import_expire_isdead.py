...
def scenario( wallets, **kw ):
    ...
    # should fail 
    resp = testlib.ysi_name_revoke( "foo.test", wallets[3].privkey, safety_checks=False )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
...
def check( state_engine ):
    ...
    global import_block, fail_blocks
    ...
    if historic_name_rec is None or len(historic_name_rec) == 0:
        print "import: no name at %s" % import_block
        return False

    if len(historic_name_rec) != 1:
        print "multiple names at %s" % import_block
        return False
    ...
    for fb in fail_blocks:
        ...
        if historic_name_rec is None or len(historic_name_rec) != 1:
            print "fail: no name at %s" % fb
            return False

        historic_name_rec = historic_name_rec[0]
        if historic_name_rec['opcode'] != 'NAME_IMPORT':
            print "operation %s succeeded at %s when it should have failed" % (historic_name_rec['opcode'], fb)
            return False
