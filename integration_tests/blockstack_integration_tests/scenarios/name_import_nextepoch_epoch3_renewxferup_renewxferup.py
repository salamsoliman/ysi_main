...
def scenario( wallets, **kw ):
    ...
    lookup = testlib.ysi_cli_lookup('foo.test')
    ...
    # renew
    resp = testlib.ysi_name_renew( 'foo.test', wallets[2].privkey, zonefile_hash='33'*20, recipient_addr=wallets[4].addr)
    if 'error' in resp:
        print resp
        return False
...
def check( state_engine ):
    ...
    # renewed (7 blocks later, starting from renewal time)
