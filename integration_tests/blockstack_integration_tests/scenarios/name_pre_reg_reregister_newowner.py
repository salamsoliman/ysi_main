...
def scenario( wallets, **kw ):
    ...
    for i in xrange(2, 5):
        resp = testlib.ysi_name_preorder( "foo.test", wallets[i].privkey, wallets[(i+1)%5].addr, safety_checks=False )
        if 'error' in resp:
        ...
        resp = testlib.ysi_name_register( "foo.test", wallets[i].privkey, wallets[(i+1)%5].addr, safety_checks=False )
        if 'error' in resp:
            print json.dumps( resp, indent=4 )
