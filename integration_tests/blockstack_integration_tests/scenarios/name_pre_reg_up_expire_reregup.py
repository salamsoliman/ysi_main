...
def scenario( wallets, **kw ):
    ...
    testlib.ysi_name_register( "foo1.test", wallets[2].privkey, wallets[3].addr)
    testlib.ysi_name_register( "foo2.test", wallets[2].privkey, wallets[3].addr)
    testlib.ysi_name_register( "foo3.test", wallets[2].privkey, wallets[3].addr)
    testlib.next_block( **kw )

    testlib.ysi_name_update( 'foo1.test', '11' * 20, wallets[3].privkey)
    testlib.ysi_name_update( 'foo2.test', '22' * 20, wallets[3].privkey)
    testlib.ysi_name_update( 'foo3.test', '33' * 20, wallets[3].privkey)
    testlib.next_block( **kw )
    ...
    testlib.ysi_name_preorder( "foo1.test", wallets[3].privkey, wallets[2].addr )
    testlib.ysi_name_preorder( "foo2.test", wallets[3].privkey, wallets[2].addr )
    testlib.ysi_name_preorder( "foo3.test", wallets[3].privkey, wallets[2].addr )
    testlib.next_block( **kw )

    testlib.ysi_name_register( "foo1.test", wallets[3].privkey, wallets[2].addr, zonefile_hash='1a' * 20)
    testlib.ysi_name_register( "foo2.test", wallets[3].privkey, wallets[2].addr, zonefile_hash='2a' * 20)
    testlib.ysi_name_register( "foo3.test", wallets[3].privkey, wallets[2].addr, zonefile_hash='3a' * 20)
    testlib.next_block( **kw )
...
def check( state_engine ):
    ...
    for i in xrange(1, 4):
        ...
        # not preordered
        preorder = state_engine.get_name_preorder( name, virtualchain.make_payment_script(wallets[3].addr), wallets[2].addr )
        if preorder is not None:
            print 'still have preorder: {}'.format(preorder)
            return False
