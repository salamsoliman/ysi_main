...
def scenario( wallets, **kw ):
    ...
    wallet_keys = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[5].privkey, wallets[3].privkey, None )
    ...
    for i in xrange(0, len(datasets)):
        ...
        put_result = ysi_client.delete_immutable( "foo.test", zonefile_hashes[i], wallet_keys=wallet_keys )
        if 'error' in put_result:
            print json.dumps(put_result, indent=4, sort_keys=True)
...
def check( state_engine ):
    ...
    for i in xrange(0, len(datasets)):
        immutable_data = ysi_client.get_immutable( "foo.test", zonefile_hashes[i] )
        ...
        immutable_data_by_name = ysi_client.get_immutable_by_name( "foo.test", "hello_world_%s" % (i+1) )
