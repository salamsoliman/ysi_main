...
def scenario( wallets, **kw ):
    ...
    atlasdb_path = kw['ysi_opts']['atlasdb_path']
    zonefile_dir = kw['ysi_opts']['zonefiles']
...
def check( state_engine ):
    ...
    for i in xrange(0, len(value_hashes)):
        ...
        if not zfinfo['tried_storage']:
            ...
            zfinfo2 = ysi.atlasdb_get_zonefile( value_hash, path=os.path.join(atlas_dir, "localhost:17000/atlas.db") )
        ...
        zfdata = ysi_client.zonefile.load_name_zonefile(name, value_hash, storage_drivers=['disk'])
        if zfdata is None:
            print "failed to load zonefile %s from disk" % value_hash
            return False

        # zonefile cached?
        cached_zonefile = ysi.lib.storage.get_cached_zonefile( value_hash, zonefile_dir=zonefile_dir )
