...
def scenario( wallets, **kw ):
    ...
    for i in xrange(0, 5):
        ...
        result = testlib.ysi_name_update( "foo.test", str(i) * 40, wallets[3 + (i%2)].privkey )
        update_hashes.append( str(i) * 40 )
        testlib.next_block( **kw )

        result = testlib.ysi_name_transfer( "foo.test", wallets[3 + ((i+1)%2)].addr, True, wallets[3 + (i%2)].privkey )
        testlib.next_block( **kw )
...
def check( state_engine ):
    ...
    update_history = ysi_client.list_update_history( name )
