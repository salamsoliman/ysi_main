...
def scenario( wallets, **kw ): 
    ...
    for count in xrange(0, 3):
        resp = testlib.ysi_namespace_preorder( "test", wallets[count+1].addr, wallets[count].privkey )
        if 'error' in resp:
        ...
        # reveal it  
        buckets = [count] * 16
        testlib.ysi_namespace_reveal( "test", wallets[count+1].addr, count + 1, count + 1, count + 1, buckets, count + 1, count + 1, wallets[count].privkey )
        testlib.next_block( **kw )
        ...
        resp = testlib.ysi_namespace_ready( "test", wallets[count+1].privkey )
        if 'error' in resp:
            print json.dumps(resp, indent=4)
