...
def scenario( wallets, **kw ):
    ...
    resp = testlib.ysi_name_renew( "foo.test", wallets[4].privkey, zonefile_hash='11' * 20 )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        return False
    ...
    # transfer 
    resp = testlib.ysi_name_transfer('foo.test', wallets[1].addr, True, wallets[4].privkey)
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        return False
    ...
    resp = testlib.ysi_name_renew( "foo.test", wallets[1].privkey, zonefile_hash='22' * 20, recipient_addr=wallets[0].addr )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
    ...
    # transfer 
    resp = testlib.ysi_name_transfer('foo.test', wallets[2].addr, True, wallets[0].privkey)
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        return False
    ...
    # renew/transfer 
    resp = testlib.ysi_name_renew( "foo.test", wallets[2].privkey, recipient_addr=wallets[1].addr )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        return False
...
def check( state_engine ):
    ...
    # renewed (7 blocks difference)
    if name_rec['last_renewed'] - 7 != name_rec['first_registered']:
