...
def scenario( wallets, **kw ):
    ...
    res = testlib.ysi_cli_app_signin("foo.test", datastore_pk, 'register.app', ['names', 'register', 'prices', 'zonefiles', 'blockchain', 'node_read', 'update'])
    if 'error' in res:
        print json.dumps(res, indent=4, sort_keys=True)
        error = True
        return 
    ...
    res = testlib.ysi_REST_call("PUT", "/v1/names/bar.test/zonefile/", ses,
    ...
    res = testlib.ysi_REST_call("GET", "/v1/names/bar.test/zonefile/?raw=1", ses )
    if 'error' in res or res['http_status'] != 200:
        res['test'] = 'Failed to get name zonefile'
        print json.dumps(res)
        return False
