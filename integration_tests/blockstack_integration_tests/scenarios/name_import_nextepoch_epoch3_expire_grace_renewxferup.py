...
def scenario( wallets, **kw ):
    ...
    # renew/xfer/update 
    resp = testlib.ysi_name_renew('foo.test', wallets[3].privkey, zonefile_hash='33'*20, recipient_addr=wallets[0].addr)
    if 'error' in resp:
        print resp
        return False
    ...
    # should go through, and be rejected (even though preordered)
    resp = testlib.ysi_name_register("foo.test", wallets[1].privkey, wallets[2].addr, zonefile_hash='44' * 20, safety_checks=False, tx_fee=500*5)
    if 'error' in resp:
        print resp
        return False
