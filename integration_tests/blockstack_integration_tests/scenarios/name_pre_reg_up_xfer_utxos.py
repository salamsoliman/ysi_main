...
def check_utxo_consumption( name_or_ns, payment_wallet, owner_wallet, data_wallet, operations, recipient_address, **kw):
    ...
    wallet = testlib.ysi_client_initialize_wallet( "0123456789abcdef", payment_wallet.privkey, owner_wallet.privkey, data_wallet.privkey )
    test_proxy = testlib.TestAPIProxy()
    ysi_client.set_default_proxy( test_proxy )
    
    # estimate the fee for the operation sequence
    fees = testlib.ysi_cli_price(name_or_ns, "0123456789abcdef", recipient_address=recipient_address, operations=operations)
    if 'error' in fees:
        return fees
    ...
    fees_utxos = testlib.ysi_cli_price(name_or_ns, "0123456789abcdef", recipient_address=recipient_address, operations=operations)
