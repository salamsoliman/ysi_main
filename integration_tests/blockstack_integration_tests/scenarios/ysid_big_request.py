...
def make_xml_call(msg_len, gzipped=False):
    ...
    xml_part_http = ("POST /RPC2 HTTP/1.1\r\n"
                     "Host: localhost:" + str(ysi.RPC_SERVER_PORT) + "\r\n"
...
def scenario( wallets, **kw ):
    ...
    s.connect(("localhost", ysi.RPC_SERVER_PORT))
    s.send(xmlmsg)
    buf = s.recv(16384)
    ...
    zonefile_hash = ysi_client.get_zonefile_data_hash(big_zonefile)
    testlib.ysi_name_update("foo.test", zonefile_hash, wallets[3].privkey)
    testlib.next_block(**kw)

    srv = xmlrpclib.ServerProxy("http://localhost:{}".format(ysi.RPC_SERVER_PORT))
    ...
    zonefile_hash_2 = ysi_client.get_zonefile_data_hash(big_zonefile_2)
    testlib.ysi_name_update("foo.test", zonefile_hash_2, wallets[3].privkey)
    testlib.next_block(**kw)
    ...
    zonefile_hash_nack = ysi_client.get_zonefile_data_hash(big_zonefile_nack)
    testlib.ysi_name_update("foo.test", zonefile_hash_nack, wallets[3].privkey)
    testlib.next_block(**kw)
