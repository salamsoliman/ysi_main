...
# TODO: do for expire_nextepoch_reregister as well;
def scenario( wallets, **kw ):
    ...
    for i in xrange(0, 3):
        res = testlib.ysi_name_preorder( "foo.test", wallets[i].privkey, wallets[i+1].addr )
        if 'error' in res:
            print res
            return False
        ...
        res = testlib.ysi_name_register( "foo.test", wallets[i].privkey, wallets[i+1].addr )
        if 'error' in res:
            print res
            return False
        ...
        res = testlib.ysi_name_preorder( "foo.test", wallets[(i+1)%5].privkey, wallets[(i+2)%5].addr )
        if 'error' in res:
            print res
            return False
        ...
        res = testlib.ysi_name_register( "foo.test", wallets[(i+1)%5].privkey, wallets[(i+2)%5].addr )
        if 'error' in res:
            print res
            return False
