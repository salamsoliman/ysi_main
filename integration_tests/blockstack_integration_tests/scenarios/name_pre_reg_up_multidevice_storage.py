...
import os
import sys
os.environ["CLIENT_STORAGE_DRIVERS"] = "ysi_server,disk"
os.environ["CLIENT_STORAGE_DRIVERS_REQUIRED_WRITE"] = "ysi_server"
...
def scenario( wallets, **kw ):
    ...
    wallet_keys_2 = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[5].privkey, wallets[6].privkey, wallets[7].privkey )
    wallet_keys = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[2].privkey, wallets[3].privkey, wallets[4].privkey )
    ...
    # put some data
    put_result = testlib.ysi_cli_put_mutable( "foo.test", "hello_world_1", json.dumps(datasets[0]), password="0123456789abcdef")
    ...
    put_result = testlib.ysi_cli_put_mutable( "foo.test", "hello_world_2", json.dumps(datasets[1]), password="0123456789abcdef")
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
    ...
    put_result = testlib.ysi_cli_put_mutable( "foo.test", "hello_world_3", json.dumps(datasets[2]), password="0123456789abcdef")
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
...
def check( state_engine ):
    ...
    try:
        ...
        profile = ysi_client.storage.parse_mutable_data( raw_profile, user_pubkey )
    ...
    for i in xrange(0, len(datasets)):
        ...
        dat = ysi_client.get_mutable( "hello_world_%s" % (i+1), blockchain_id="foo.test" )
        if dat is None:
    ...
    profile, zonefile = ysi_client.get_profile('foo.test')
    if profile is None:
        print 'No profile'
        return False
