...
def setup_env(session, blockchain_id):
    ...
    ysi_client.set_secret("BLOCKSTACK_API_SESSION", session)
    res = testlib.ysi_test_setenv("TEST_BLOCKSTACK_TEST_INDEXED_STORAGE", "1")
    if 'error' in res:
        print json.dumps(res, indent=4, sort_keys=True)
        return False

    res = testlib.ysi_test_setenv("TEST_BLOCKSTACK_TEST_DISK_ROOT", "/tmp/ysi-integration-test-storage-{}".format(blockchain_id))
    if 'error' in res:
        print json.dumps(res, indent=4, sort_keys=True)
        return False
...
def target_datastore(blockchain_id):
    ...
    os.environ['TEST_BLOCKSTACK_TEST_DISK_ROOT'] = '/tmp/ysi-integration-test-storage{}'.format(blockchain_id)
...
def write_datastore(blockchain_id, datastore_pk, iteration):
    ...
    for dpath in ['/file1', '/file2', '/dir1/file3', '/dir1/dir3/file4', '/dir1/dir3/dir4/file5']:
        ...
        res = testlib.ysi_cli_datastore_putfile( blockchain_id, datastore_pk, dpath, data, ses)
        if 'error' in res:
...
def clear_datastore_files(blockchain_id, datastore_pk):
    ...
    for dpath in ['/file1', '/file2', '/dir1/file3', '/dir1/dir3/file4', '/dir1/dir3/dir4/file5']:
        ...
        res = testlib.ysi_cli_datastore_deletefile( blockchain_id, datastore_pk, dpath, ses)
        if 'error' in res:
...
def clear_datastore_directories(blockchain_id, datastore_pk):
    ...
    for dpath in ['/dir1/dir3/dir4', '/dir1/dir3', '/dir2', '/dir1']:
        ...
        res = testlib.ysi_cli_datastore_rmdir( blockchain_id, datastore_pk, dpath, ses)
        if 'error' in res:
...
def setup_storage_dirs(blockchain_ids):
    ...
    for blockchain_id in blockchain_ids:
        if not os.path.exists("/tmp/ysi-integration-test-storage-{}".format(blockchain_id)):
            os.makedirs("/tmp/ysi-integration-test-storage-{}/immutable".format(blockchain_id))
            os.makedirs("/tmp/ysi-integration-test-storage-{}/mutable".format(blockchain_id))
...
def scenario( wallets, **kw ):
    ...
    res = testlib.ysi_REST_call('POST', '/v1/node/drivers/storage/test?index=1&force=1', None, api_pass='ysi_integration_test_api_password')
