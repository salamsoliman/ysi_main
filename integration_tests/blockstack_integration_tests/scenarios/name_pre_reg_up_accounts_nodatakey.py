...
def scenario( wallets, **kw ):
    ...
    # make an account 
    res = testlib.ysi_cli_put_account("foo.test", "serviceFoo", "serviceFooID", "foo://bar.com", None, extra_data='foofield=foo!', wallet_keys=wallet_keys )
    if 'error' in res:
    ...
    res = testlib.ysi_cli_put_account("foo.test", "deletedService", "deletedServiceID", "foo://deleted", None, extra_data='barfield=bar!', wallet_keys=wallet_keys ) 
    if 'error' in res:
    ...
    # delete an account 
    res = testlib.ysi_cli_delete_account("foo.test", "deletedService", "deletedServiceID", None, wallet_keys=wallet_keys )
    if 'error' in res:
...
def check( state_engine ):
    ...
    for i in xrange(0, len(names)):
        ...
        accounts = testlib.ysi_cli_list_accounts( "foo.test" )
        if len(accounts) != 1:
        ...
        account = accounts[0]
        on_file_account = testlib.ysi_cli_get_account( "foo.test", "serviceFoo", "serviceFooID" )
