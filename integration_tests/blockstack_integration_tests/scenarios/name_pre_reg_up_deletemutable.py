...
def scenario( wallets, **kw ):
    ...
    put_result = testlib.ysi_cli_put_mutable("foo.test", "hello_world_1", json.dumps(datasets[0], sort_keys=True), password="0123456789abcdef")
    ...
    put_result = testlib.ysi_cli_put_mutable("foo.test", "hello_world_2", json.dumps(datasets[1], sort_keys=True), password="0123456789abcdef")
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False

    put_result = testlib.ysi_cli_put_mutable("foo.test", "hello_world_3", json.dumps(datasets[2], sort_keys=True), password="0123456789abcdef")
    if 'error' in put_result:
        print json.dumps(put_result, indent=4, sort_keys=True)
        return False
    ...
    for i in xrange(0, len(datasets)):
        delete_result = testlib.ysi_cli_delete_mutable("foo.test", "hello_world_%s" % (i+1), password="0123456789abcdef")
...
def check( state_engine ):
    ...
    for i in xrange(0, len(datasets)):
        ...
        dat = testlib.ysi_cli_get_mutable( "foo.test", "hello_world_%s" % (i+1) )
