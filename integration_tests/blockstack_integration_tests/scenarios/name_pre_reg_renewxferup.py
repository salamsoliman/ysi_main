...
def scenario( wallets, **kw ):
    ...
    resp = testlib.ysi_name_renew( "foo.test", wallets[3].privkey, zonefile_hash='11' * 20 )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
    ...
    # transfer 
    resp = testlib.ysi_name_renew( "foo.test", wallets[0].privkey, recipient_addr=wallets[1].addr )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
