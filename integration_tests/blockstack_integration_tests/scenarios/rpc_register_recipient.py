...
def scenario( wallets, **kw ):
    ...
    zonefile_txt = ysi_zones.make_zone_file( zonefile, origin='foo.test', ttl=3600 )

    wallet = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[2].privkey, wallets[3].privkey, wallets[4].privkey )
    resp = testlib.ysi_cli_register( "foo.test", "0123456789abcdef", zonefile=zonefile_txt, recipient_address=wallets[4].addr )
    if 'error' in resp:
...
def check( state_engine ):
    ...
    # have a zonefile 
    zonefile = testlib.ysi_get_zonefile( name_rec['value_hash'] )
    ...
    if ysi_client.hash_zonefile( zonefile ) != name_rec['value_hash']:
        print "wrong zonefile: %s != %s" % (ysi_client.hash_zonefile(zonefile), name_rec['value_hash'])
        return False

    # zonefile has the right data public key 
    zonefile_pubk = ysi_client.user.user_zonefile_data_pubkey( zonefile )
    ...
    zonefile_urls = ysi_client.user.user_zonefile_urls( zonefile )
    driver_urls = ysi_client.storage.make_mutable_data_urls( 'foo.test', use_only=['dht', 'disk'] )
    ...
    profile = testlib.ysi_get_profile( "foo.test" )
