...
def scenario( wallets, **kw ):
    ...
    for i in xrange(2, 4):
        resp = testlib.ysi_name_preorder( "foo.test", wallets[i].privkey, wallets[(i+1)%11].addr, consensus_hash=consensus_hash, safety_checks=False )
        if 'error' in resp:
        ...
        resp = testlib.ysi_name_transfer( "foo.test", wallets[i].addr, True, wallets[(i+1)%11].privkey, consensus_hash=consensus_hash, safety_checks=False )
        testlib.next_block( **kw )
        if 'error' in resp:
            print json.dumps( resp, indent=4 )
        ...
        resp = testlib.ysi_name_update( "foo.test", ("%02x" % i) * 20, wallets[i].privkey, consensus_hash=consensus_hash, safety_checks=False )
        testlib.next_block( **kw )
        if 'error' in resp:
            print json.dumps( resp, indent=4 )
