...
def scenario( wallets, **kw ):
    ...
    assert ysi_client.user.put_immutable_data_zonefile( zonefile, "testdata", ysi_client.get_data_hash("testdata") )
    ...
    # store to queue
    res = ysi_client.backend.queue.queue_append("update", "foo.test", txhash, payment_address=wallets[2].addr, owner_address=wallets[3].addr,
    ...
    ysi_client.backend.queue.queuedb_remove("update", "foo.test", txhash, path=queuedb_path )
    ...
    # store a new zonefile
    zonefile = ysi_client.zonefile.make_empty_zonefile( "foo.test", data_pubkey )
    ysi_client.user.put_immutable_data_zonefile( zonefile, "testdata2", ysi_client.get_data_hash("testdata") )

    zonefile_txt = ysi_zones.make_zone_file( zonefile )
    zonefile_hash = ysi_client.storage.get_zonefile_data_hash( zonefile_txt )

    # store locally 
    res = ysi_client.zonefile.store_name_zonefile("foo.test", zonefile, None, storage_drivers=['disk'])
    if 'error' in res:
