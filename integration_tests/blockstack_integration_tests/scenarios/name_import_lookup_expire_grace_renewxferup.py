...
def scenario( wallets, **kw ):
    ...
    driver_urls = ysi_client.storage.make_mutable_data_urls('foo.test', use_only=['dht', 'disk'])
    zonefile = ysi_client.zonefile.make_empty_zonefile('foo.test', wallets[4].pubkey_hex, urls=driver_urls)
    zonefile_txt = ysi_zones.make_zone_file( zonefile, origin='foo.test', ttl=4200 )
    ...
    resp = testlib.ysi_name_import( "foo.test", wallets[3].addr, zonefile_hash, wallets[1].privkey )
    if 'error' in resp:
        print json.dumps( resp, indent=4 )
        return False
    ...
    # broadcast zonefile 
    res = testlib.ysi_cli_sync_zonefile('foo.test', zonefile_string=zonefile_txt)
    if 'error' in res:
        print res
        return False
    ...
    rc = ysi_client.profile.put_profile('foo.test', user_profile, blockchain_id='foo.test',
    ...
    # make a zonefile and a profile 
    driver_urls = ysi_client.storage.make_mutable_data_urls('foo.test', use_only=['dht', 'disk'])
    new_zonefile = ysi_client.zonefile.make_empty_zonefile('foo.test', wallets[4].pubkey_hex, urls=driver_urls)
    new_zonefile_txt = ysi_zones.make_zone_file( new_zonefile, origin='foo.test', ttl=4200 )
    ...
    rc = ysi_client.profile.put_profile('foo.test', new_user_profile, blockchain_id='foo.test',
    ...
    # renew/xfer/update 
    resp = testlib.ysi_name_renew('foo.test', wallets[3].privkey, zonefile_hash=new_zonefile_hash, recipient_addr=wallets[0].addr)
    if 'error' in resp:
        print resp
        return False
