...
def scenario( wallets, **kw ):
    ...
    wallet = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[3].privkey, wallets[0].privkey, wallets[1].privkey, exception=False )
    if 'error' not in wallet:
        print "wallet initialized\n%s" % json.dumps(wallet, indent=4, sort_keys=True)
        return False

    wallet = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[0].privkey, wallets[4].privkey, wallets[1].privkey, exception=False )
    if 'error' not in wallet:
        print "wallet initialized\n%s" % json.dumps(wallet, indent=4, sort_keys=True)
        return False

    wallet = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[3].privkey, wallets[4].privkey, wallets[1].privkey, exception=False )
    if 'error' not in wallet:
        print "wallet initialized\n%s" % json.dumps(wallet, indent=4, sort_keys=True)
        return False

    # should fail if data key is multisig
    wallet = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[0].privkey, wallets[1].privkey, wallets[3].privkey, exception=False )
    if 'error' not in wallet:
        print "wallet initialized\n%s" % json.dumps(wallet, indent=4, sort_keys=True)
        return False

    # should succeed 
    wallet = testlib.ysi_client_initialize_wallet( "0123456789abcdef", wallets[0].privkey, wallets[1].privkey, wallets[1].privkey, exception=False )
    if 'error' in wallet:
