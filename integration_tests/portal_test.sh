...
API_PASSWORD="$1"
TEST_OUTPUT=/tmp/ysi-portal.log

export BLOCKSTACK_CLIENT_CONFIG="/tmp/ysi-run-scenario.ysi_integration_tests.scenarios.name_preorder_register_portal/client/client.ini"
...
ysi-test-scenario --interactive 5 ysi_integration_tests.scenarios.name_preorder_register_portal > "$TEST_OUTPUT" 2>&1 &
TEST_PID="$!"
...
while true; do 
    ERR="$(ysi -y whois foo.id | sed -r '1,2d' | jq '.error' | grep -v 'null')"
    if [ -n "$ERR" ]; then 
        # not there yet
        sleep 1
        echo -n '.'
...
# start API server
ysi -y -p 0123456789abcdef api start-foreground
