...
def app_publish( dev_blockchain_id, app_domain, app_method_list, app_index_uris, app_index_file, app_driver_hints=[], data_privkey=None, proxy=None, wallet_keys=None, config_path=CONFIG_PATH ):
    ...
    * replicate the list of URIs and the list of methods to ".ysi" via each of the client's storage drivers.
    ...
    config_data_id = storage.make_fq_data_id(app_domain, '.ysi')
...
def app_get_config( blockchain_id, app_domain, data_pubkey=None, proxy=None, config_path=CONFIG_PATH ):
    ...
    # go get config
    res = data.get_mutable( ".ysi", [app_domain], data_pubkey=data_pubkey, proxy=proxy, config_path=config_path, blockchain_id=blockchain_id )
    if 'error' in res:
