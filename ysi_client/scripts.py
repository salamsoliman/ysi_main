...
def tx_get_subsidy_info(ysi_tx, fee_cb, max_fee, subsidy_key_info, utxo_client, subsidy_address=None, tx_fee=0):
    """
    ...
    * calculate the paying key's address (payer_address)
    ...
    tx_inputs, tx_outputs = deserialize_tx(ysi_tx)
...
def tx_make_subsidizable(ysi_tx, fee_cb, max_fee, subsidy_key_info, utxo_client, tx_fee=0,
    ...
    * Sign our inputs with SIGHASH_ANYONECANPAY (if subsidy_key_info is not None)
    ...
    subsidy_info = tx_get_subsidy_info(ysi_tx, fee_cb, max_fee, subsidy_key_info, utxo_client, tx_fee=tx_fee, subsidy_address=subsidy_address)
    ...
    def _make_subsidized_from(inputs, _tx_fee):
        ...
        subsidized_tx = tx_extend(ysi_tx, inputs, [subsidy_output])
