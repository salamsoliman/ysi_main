...
from ysi_client import storage, subdomains
...
def load_legacy_user_profile(name, expected_hash):
    ...
    assert ysi_profiles.is_profile_in_legacy_format(data_json)
    new_profile = ysi_profiles.get_person_from_legacy_format(data_json)
    return new_profile
...
def _get_person_profile(name, proxy=None):
    ...
    if ysi_profiles.is_profile_in_legacy_format(profile):
        return {'error': 'Legacy profile'}

    person = None
    try:
        person = ysi_profiles.Person(profile)
    except Exception as e:
        log.exception(e)
