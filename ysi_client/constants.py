...
DEFAULT_BLOCKSTACKD_PORT = 6263  # ysi indexer port
DEFAULT_BLOCKSTACKD_SERVER = 'node.ysi.org'
...
BLOCKSTACK_DEFAULT_STORAGE_DRIVERS = 'disk,gaia_hub,dropbox,s3,ysi_resolver,http,dht'
...
MAX_RPC_LEN = RPC_MAX_ZONEFILE_LEN * 110    # maximum ysid RPC length--100 zonefiles with overhead
...
if not BLOCKSTACK_TEST:
    ...
    if CONFIG_PATH is None:
        # default
        CONFIG_DIR = os.path.expanduser("~/.ysi")
...
for i in range(1, NUM_EPOCHS + 1):
    ...
    ysi_epoch_end_block = os.environ.get('BLOCKSTACK_EPOCH_{}_END_BLOCK'.format(i), None)
    if ysi_epoch_end_block is not None and BLOCKSTACK_TEST is not None:
        exec('EPOCH_{}_END_BLOCK = int({})'.format(i, ysi_epoch_end_block))
        if DEBUG:
...
DEFAULT_BLOCKCHAIN_READER = 'ysi_utxo'
DEFAULT_BLOCKCHAIN_WRITER = 'ysi_utxo'
