...
# default API endpoint proxy to ysid
default_proxy = None


class BlockstackRPCClient(object):
    """
    RPC client for the ysi server
    """
...
def get_default_proxy(config_path=CONFIG_PATH):
    """
    Get the default API proxy to ysi.
    """
    ...
    ysi_server, ysi_port = conf['server'], conf['port']
    protocol = conf['protocol']

    if os.environ.get("BLOCKSTACK_TEST") == "1":
        # we'd better be using the test port
        if ysi_port != 16264:
            log.warning("Invalid port {} loaded from {}, at\n{}".format(ysi_port, config_path, ''.join(traceback.format_stack())))

    log.debug('Default proxy to {}://{}:{}'.format(protocol, ysi_server, ysi_port))

    proxy = client.session(conf=conf, server_host=ysi_server, server_port=ysi_port,
