...
def get_metadata_dir(conf, config_path=CONFIG_PATH):
    ...
    assert metadata_dir, "Config file is missing ysi_client.metadata"
...
def is_obsolete_zonefile(user_zonefile):
    return (
        ysi_profiles.is_profile_in_legacy_format(user_zonefile) or
...
def list_immutable_data_history(name, data_id, current_block=None, proxy=None):
    ...
    for zf in zonefiles:
        ...
        # try to parse
        try:
            zf = ysi_zones.parse_zone_file(zf)
...
if __name__ == "__main__":
    ...
    rpc = ysi_client.rpc.local_api_connect(api_session=ses)
    assert rpc
