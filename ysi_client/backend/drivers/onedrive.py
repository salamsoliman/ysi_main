...
log = get_logger('ysi-client-onedrive')
...
def get_ysi_folder_id(client, folder_name):
    """
    Get the ysi-designated folder id
...
def get_chunk_via_onedrive(drive, data_id):
    ...
    if ONEDRIVE_FOLDER_ID is None:
        ...
        fid = get_ysi_folder_id(drive, ONEDRIVE_FOLDER_NAME)
        if fid is None:
            return None 
...
def put_chunk( drive, name, chunk_buf ):
    ...
    if ONEDRIVE_FOLDER_ID is not None:
        ...
        assert ONEDRIVE_FOLDER_NAME
        ONEDRIVE_FOLDER_ID = get_ysi_folder_id(drive, ONEDRIVE_FOLDER_NAME)
        if ONEDRIVE_FOLDER_ID is None:
            return False
...
def make_mutable_url( data_id ):
    ...
    url = 'https://onedrive.live.com/ysi/{}'.format(data_id)
    return url
