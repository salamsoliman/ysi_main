...
if os.environ.get("BLOCKSTACK_TEST", None) == "1":
    ...
    SERVER_NAME = "node.ysi.org"
    SERVER_PORT = 6264

log = get_logger("ysi-storage-driver-ysi-server")
...
def get_data( data_id, zonefile=False, fqu=None ):
    ...
    if zonefile:
        ...
        if not is_zonefile_hash(data_id):
            ...
            if not ysi_client.is_name_valid(data_id):
                log.error("Not a valid name or zone file hash: {}".format(data_id))
                return None

            # find zonefile hash
            res = ysi_client.proxy.get_name_blockchain_record(data_id, proxy=ses)
            if 'error' in res:
        ...
        res = ysi_client.proxy.get_zonefiles([zonefile_hash], proxy=ses)
        if 'error' in res:
    ...
    elif ysi_client.is_name_valid(data_id):
        ...
        if ysi_client.proxy.json_is_error(res):
...
def put_data( data_id, data_txt, zonefile=False, fqu=None, profile=False ):
    ...
    if zonefile:
        ...
        if len(data_txt) >= ysi.RPC_MAX_PROFILE_LEN:
        ...
        # data_txt must be a sufficiently small JSON blob
        if len(data_txt) >= ysi.RPC_MAX_DATA_LEN:
            log.error("Data is too big")
            return False
...
def storage_init(conf, **kw):
    ...
    if SERVER_NAME is None or SERVER_PORT is None:
        log.error("Config file '%s': section 'ysi_server_storage' is missing 'host' and/or 'port'")
        return False
...
if __name__ == "__main__":
    ...
    import ysi_client
    config = ysi_client.get_config(config_path)
