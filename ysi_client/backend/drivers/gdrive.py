...
log = get_logger("ysi-storage-drivers-gdrive")
...
def get_gdrive_handle(settings_path=None, folder_name=None):
    ...
    GDRIVE_HANDLE = drive
    GDRIVE_FOLDER_ID = get_ysi_folder_id(GDRIVE_HANDLE, folder_name)

    return drive


def get_ysi_folder_id(drive, folder):
    """
    Make sure the ysi-specific data folder exists.
...
def gdrive_put_chunk( dvconf, chunk_buf, name ):
    ...
    if GDRIVE_FOLDER_ID is not None:
        ...
        GDRIVE_FOLDER_ID = get_ysi_folder_id(drive, GDRIVE_FOLDER_NAME)
        fid = GDRIVE_FOLDER_ID
...
def gdrive_delete_chunk(dvconf, name):
    ...
    if GDRIVE_FOLDER_ID is None:
        fid = get_ysi_folder_id(drive, GDRIVE_FOLDER_NAME)
