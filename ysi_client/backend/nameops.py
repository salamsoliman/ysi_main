...
from ..proxy import getinfo as ysi_getinfo
from ..proxy import get_name_cost as ysi_get_name_cost
from ..proxy import get_namespace_blockchain_record as ysi_get_namespace_blockchain_record
...
def get_consensus_hash( proxy, config_path=CONFIG_PATH ):
    ...
    delay = 1.0
    while True:
        ysi_info = ysi_getinfo( proxy=proxy )
        if 'error' in ysi_info:
            log.error("Blockstack server did not return consensus hash: {}".format(ysi_info['error']))
            time.sleep(delay)
        ...
        try:
            last_block_processed = int(ysi_info['last_block_processed'])
            last_block_seen = int(ysi_info['last_block_seen'])
            consensus_hash = ysi_info['consensus']
        except Exception, e:
            log.exception(e)
...
def get_namespace_burn_address(fqu, proxy):
    ...
    namespace_info = ysi_get_namespace_blockchain_record(namespace_id, proxy=proxy)
    ...
    if (namespace_info['version'] & NAMESPACE_VERSION_PAY_TO_CREATOR) != 0:
        ...
        receive_fees_period = ysi.get_epoch_namespace_receive_fees_period(block_height, namespace_id)
        if namespace_info['reveal_block'] + receive_fees_period >= block_height:
            ...
            burn_address = ysi.BLOCKSTACK_BURN_ADDRESS
...
def async_register(fqu, payment_privkey_info, owner_privkey_info, name_data={}, owner_address=None,
    ...
    if ysi.EPOCH_FEATURE_OP_REGISTER_UPDATE in ysi.get_epoch_features(block_height):
