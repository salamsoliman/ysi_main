    ...
    * can only change owner if we're in epoch 3+
    ...
    def _register_can_change_zonefile_hash(zonefile_hash):
        ...
        if zonefile_hash is not None:
            ...
            epoch_features = ysi.get_epoch_features(indexer_info['last_block_seen']+1)
            if ysi.EPOCH_FEATURE_OP_REGISTER_UPDATE not in epoch_features:
                # not active yet
                return {'error': 'NAME_REGISTRATION cannot set value hashes in this epoch (block {})'.format(indexer_info['last_block_seen'])}

            log.debug("Epoch feature '{}' is active!".format(ysi.EPOCH_FEATURE_OP_REGISTER_UPDATE))
    ...
    def _renewal_can_change_zonefile_hash(zonefile_hash):
        ...
        if zonefile_hash is not None:
            ...
            # +1, so we consider the next block to be formed
            epoch_features = ysi.get_epoch_features(indexer_info['last_block_seen']+1)
            if ysi.EPOCH_FEATURE_OP_RENEW_TRANSFER_UPDATE not in epoch_features:
                # not active yet
                return {'error': 'NAME_RENEWAL cannot set value hashes in this epoch (block {})'.format(indexer_info['last_block_seen'])}

            log.debug("Epoch feature '{}' is active!".format(ysi.EPOCH_FEATURE_OP_RENEW_TRANSFER_UPDATE))
    ...
    def _is_burn_address_correct(fqu, burn_addr):
        ...
        if burn_addr is not None:
            ...
            epoch_features = ysi.get_epoch_features(block_height)
            if ysi.EPOCH_FEATURE_NAMESPACE_BURN_TO_CREATOR not in epoch_features and virtualchain.address_reencode(burn_addr) != virtualchain.address_reencode(ysi.BLOCKSTACK_BURN_ADDRESS):
                # not active yet 
                return {'error': 'cannot burn to namespace burn address (not allowed in this epoch)'}

            # what's the namespace burn address?
            nsid = ysi.get_namespace_from_name(fqu)
            ...
            if (ns_info['version'] & NAMESPACE_VERSION_PAY_TO_BURN) != 0:
                ...
                ns_burn_address = ysi.BLOCKSTACK_BURN_ADDRESS
                ...
                receive_fees_period = ysi.get_epoch_namespace_receive_fees_period(block_height, nsid)
    ...
    def _is_name_outside_grace_period(fqu):
        ...
        grace_info = ysi.BlockstackDB.get_name_deadlines(name_rec, namespace_rec, block_number)
    ...
    def _is_name_import_key(fqu_or_ns, import_privkey):
        ...
        * if this is the first name imported, then the key must be equal to the reveal key
        ...
        nsid = fqu_or_ns
        if '.' in fqu_or_ns:
            nsid = ysi.get_namespace_from_name(fqu_or_ns)
        ...
        keychain_path = ysi.BlockstackDB.get_import_keychain_path(keychain_dir, nsid)
        ...
        child_addrs = ysi.BlockstackDB.build_import_keychain( nsid, ecdsa_private_key(import_privkey).public_key().to_hex(), keychain_dir=keychain_dir )
