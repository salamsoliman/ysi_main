...
import config as ysi_constants
import backend
...
class BlockstackAPIEndpointHandler(SimpleHTTPRequestHandler):
    ...
    def POST_names( self, ses, path_info ):
        ...
        resp = {
            ...
            'message': 'Name queued for registration.  The process takes several hours.  You can check the status with `ysi info`.',
        }
    ...
    def GET_name_history(self, ses, path_info, name ):
        ...
        return 500 on failure to query ysi server
        """
        qs_values = path_info['qs_values']
    ...
    def PUT_name_transfer( self, ses, path_info, name ):
        ...
        resp = {
            'success': True,
            'transaction_hash': res['transaction_hash'],
            'message': 'Name queued for transfer.  The process takes ~1 hour.  You can check the status with `ysi info`.',
        }
    ...
    def PUT_name_zonefile( self, ses, path_info, name ):
        ...
        resp = {
            ...
            'transaction_hash': res['transaction_hash'],
            'message': 'Name queued for update.  The process takes ~1 hour.  You can check the status with `ysi info`.',
        }
    ...
    def DELETE_name( self, ses, path_info, name ):
        ...
        resp = {
            'success': True,
            'transaction_hash': res['transaction_hash'],
            'message': 'Name queued for revocation.  The process takes ~1 hour.  You can check the status with `ysi info`.'
        }
    ...
    def GET_name_zonefile_by_hash( self, ses, path_info, name, zonefile_hash ):
        ...
        conf = ysi_config.get_config(self.server.config_path)

        ysi_server = conf['server']
        ysi_port = conf['port']
        ysi_hostport = '{}:{}'.format(ysi_server, ysi_port)
        ...
        resp = proxy.get_zonefiles( ysi_hostport, [str(zonefile_hash)] )
        if json_is_error(resp):
            self._reply_json({'error': resp['error']}, status_code=500)
            return
    ...
    def GET_namespace_names( self, ses, path_info, namespace_id ):
        ...
        Reply 404 if the namespace doesn't exist
        Reply 500 for any error in talking to the ysi server
        """
    ...
    def GET_node_config( self, ses, path_info ):
        ...
        conf = None
        try:
            conf = ysi_config.read_config_file(config_path=self.server.config_path)
            assert conf
        except Exception as e:
            log.exception(e)
    ...
    def POST_node_config( self, ses, path_info, section ):
        ...
        try:
            res = ysi_config.write_config_section(self.server.config_path, section, conf_items)
            assert res
        except Exception as e:
            if BLOCKSTACK_DEBUG:
                log.exception(e)
    ...
    def DELETE_node_config_section( self, ses, path_info, section ):
        ...
        """
        res = ysi_config.delete_config_section(self.server.config_path, section)
        if not res:
    ...
    def DELETE_node_config_field( self, ses, path_info, section, field ):
        ...
        Return 500 on failure
        """
        res = ysi_config.delete_config_field(self.server.config_path, section, field)
        if not res:
    ...
    def POST_node_storage_driver_config(self, ses, path_info, driver_name):
        ...
        * force (0, 1): (DANGEROUS) forcibly set up the index, even if it appears to already exist
        ...
        if driver_request is not None and 'error' not in driver_request:
            ...
            try:
                rc = ysi_config.write_config_section(self.server.config_path, driver_name, driver_config)
                assert rc
            except Exception as e:
                if BLOCKSTACK_DEBUG:
                    log.exception(e)
    ...
    def GET_blockchain_ops( self, ses, path_info, blockchain_name, blockheight ):
        ...
        Reply 500 for any error we have in talking to the ysi server
        """
    ...
    def GET_help(self, session, path_info):
        ...
        """
        template = """<html><header></header><body>Blockstack RESTful API<br><a href="https://github.com/ysi/ysi-core/tree/master/api">Documentation</a></body></html>"""
...
class BlockstackAPIEndpoint(SocketServer.ThreadingMixIn, SocketServer.TCPServer):
    ...
    def register_api_functions(self, config_path):
        ...
        # load methods
        all_methods = ysi_client.get_cli_methods()
    ...
    def __init__(self, api_pass, wallet_keys, host='localhost', port=ysi_constants.DEFAULT_API_PORT,
        ...
        conf = ysi_config.get_config(path=config_path)
        assert conf
...
class BlockstackAPIEndpointClient(object):
    """
    Client for ysi's local API endpoint.
    ...
    def check_version(self):
        ...
        if res.get('version', None) != SERIES_VERSION:
            ...
            return {'error': 'Obsolete API server (version {}).  Please restart it with `ysi api restart`.'.format(res.get('version', '<unknown>'))}
...
def make_local_api_server(api_pass, portnum, wallet_keys, api_bind=None, config_path=ysi_constants.CONFIG_PATH, plugins=None):
    """
...
def is_api_server(config_dir=ysi_constants.CONFIG_DIR):
    """
...
def local_api_connect(api_pass=None, api_session=None, config_path=ysi_constants.CONFIG_PATH, api_host=None, api_port=None):
    """
...
def local_api_action(command, password=None, api_pass=None, config_dir=ysi_constants.CONFIG_DIR):
    """
    ...
    * restart: stop and start the API endpoint
    ...
    if command == 'start-foreground':
        ...
        # use the RPC runner
        argv = [sys.executable, '-m', 'ysi_client.rpc_runner', str(command), str(api_port), str(config_dir)]
...
def local_api_pidfile_path(config_dir=ysi_constants.CONFIG_DIR):
    """
...
def local_api_logfile_path(config_dir=ysi_constants.CONFIG_DIR):
    """
...
def backup_api_logfile(config_dir=ysi_constants.CONFIG_DIR):
    """
    Back up the logfile
    """
...
def local_api_start( port=None, host=None, config_dir=ysi_constants.CONFIG_DIR, foreground=False, api_pass=None, password=None):
    """
    ...
    import ysi_client
    from ysi_client.wallet import load_wallet
    from ysi_client.client import session
...
def local_api_stop(config_dir=ysi_constants.CONFIG_DIR):
    """
...
def local_api_status(config_dir=ysi_constants.CONFIG_DIR):
    """
