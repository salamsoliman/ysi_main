...
def get_network_log_handler(api_password=None, name=None, scheme="http", host="localhost", port=LOG_NETWORK_PORT):
    ...
    if api_password is None:
        ...
        try:
            if p.has_section('ysi-client'):
                if p.get('ysi-client', 'api_password') is not None:
                    api_password = p.get('ysi_client', 'api_password')
        except:
            return None
...
def get_logger(name="ysi-client", debug=DEBUG):
