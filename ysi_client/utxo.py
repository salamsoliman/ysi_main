...
from backend.utxo.ysi_core import BlockstackCoreUTXOClient
...
from backend.utxo.ysi_explorer import BlockstackExplorerClient, BLOCKSTACK_EXPLORER_URL
from backend.utxo.ysi_utxo import BlockstackUTXOClient, BLOCKSTACK_UTXO_URL

from backend.utxo.ysi_core import get_unspents as ysi_core_get_unspents
from backend.utxo.ysi_core import broadcast_transaction as ysi_core_broadcast_transaction
...
from backend.utxo.ysi_explorer import get_unspents as ysi_explorer_get_unspents
from backend.utxo.ysi_explorer import broadcast_transaction as ysi_explorer_broadcast_transaction

from backend.utxo.ysi_utxo import get_unspents as ysi_utxo_get_unspents
from backend.utxo.ysi_utxo import broadcast_transaction as ysi_utxo_broadcast_transaction

from constants import TX_MIN_CONFIRMATIONS

DEBUG = True
FIRST_BLOCK_MAINNET = 373601        # well-known value for ysi-core; doesn't ever change


SUPPORTED_UTXO_PROVIDERS = [ "blockcypher", "blockchain_info", "bitcoind_utxo", "ysi_core", "ysi_explorer", "ysi_utxo" ]
...
SUPPORTED_UTXO_PROMPT_MESSAGES = {
    ...
    'ysi_core': 'Please enter your Blockstack Core node info.',
    'ysi_explorer': 'Please enter your Blockstack Explorer info.',
    'ysi_utxo': 'Please enter your Blockstack UTXO service info',
}
...
def default_utxo_provider_opts( utxo_provider, config_file=None ):
   ...
   elif utxo_provider == "ysi_core":
       return default_ysi_core_opts( config_file=config_file )
    
   elif utxo_provider == "ysi_explorer":
       return default_ysi_explorer_opts( config_file=config_file )

   elif utxo_provider == 'ysi_utxo':
       return default_ysi_utxo_opts( config_file=config_file )
...
def default_ysi_core_opts( config_file=None ):
   """
   ...
   provider_secs = find_service_provider_sections(config_file, 'ysi_core')
   if len(provider_secs) > 0:
       provider_sec = provider_secs[0]
   ...
   # strip Nones
   for (k, v) in ysi_core_opts.items():
      if v is None:
         del ysi_core_opts[k]

   ysi_core_opts['utxo_provider'] = 'ysi_core'
   return ysi_core_opts


def default_ysi_explorer_opts( config_file=None ):
    """
    ...
    provider_secs = find_service_provider_sections(config_file, 'ysi_explorer')
    if len(provider_secs) > 0:
        provider_sec = provider_secs[0]
    ...
    # strip nones 
    for (k, v) in ysi_explorer_opts.items():
        if v is None:
            del ysi_explorer_opts[k]

    ysi_explorer_opts['utxo_provider'] = 'ysi_explorer'
    return ysi_explorer_opts


def default_ysi_utxo_opts( config_file=None ):
    """
    ...
    provider_secs = find_service_provider_sections(config_file, 'ysi_utxo')
    if len(provider_secs) > 0:
        provider_sec = provider_secs[0]
    ...
    ysi_explorer_opts['utxo_provider'] = 'ysi_utxo'
    return ysi_explorer_opts
...
def get_unspents(address, blockchain_client, use_builtin=True):
    ...
    if isinstance(blockchain_client, BlockcypherClient):
        ...
        return ysi_core_get_unspents(address, blockchain_client)
    elif isinstance(blockchain_client, BlockstackExplorerClient):
        return ysi_explorer_get_unspents(address, blockchain_client)
    elif isinstance(blockchain_client, BlockstackUTXOClient):
        return ysi_utxo_get_unspents(address, blockchain_client)
...
def broadcast_transaction(hex_tx, blockchain_client):
    ...
    if isinstance(blockchain_client, BlockcypherClient):
        ...
        return ysi_core_broadcast_transaction(hex_tx, blockchain_client)
    elif isinstance(blockchain_client, BlockstackExplorerClient):
        return ysi_explorer_broadcast_transaction(hex_tx, blockchain_client)
    elif isinstance(blockchain_client, BlockstackUTXOClient):
        return ysi_utxo_broadcast_transaction(hex_tx, blockchain_client)
