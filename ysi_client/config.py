...
# NOTE: duplicated from ysi-core and streamlined.
...
def configure(config_file=CONFIG_PATH, force=False, interactive=True, set_migrate=False):
    """
    Configure ysi-client:  find and store configuration parameters to the config file.
    ...
    ysi_opts = {}
    ysi_opts_defaults = all_opts['ysi-client']
    ...
    if not force:
        # defaults
        ysi_opts = copy.deepcopy(ysi_opts_defaults)

    ysi_opts, missing_ysi_opts, num_ysi_opts_prompted = find_missing(
        ysi_message,
        ysi_params,
        ysi_opts,
        ysi_opts_defaults,
    ...
    blockchain_reader = ysi_opts.get('blockchain_reader')
    ...
    blockchain_writer = ysi_opts.get('blockchain_writer')
    ...
    missing_opts = [missing_bitcoin_opts, missing_writer_opts, missing_reader_opts, missing_ysi_opts]
    ...
    if any([num_bitcoind_prompted, num_reader_opts_prompted, num_writer_opts_prompted, num_ysi_opts_prompted]):
    ...
    ret['ysi-client']['path'] = config_file
    if config_file is not None:
        ret['ysi-client']['dir'] = os.path.dirname(config_file)
    else:
        ret['ysi-client']['dir'] = None
...
def read_config_file(config_path=CONFIG_PATH, set_migrate=False):
    ...
    if config_path is None or not os.path.exists(config_path):
        ...
        parser = SafeConfigParser()
        parser.add_section('ysi-client')
        parser.set('ysi-client', 'server', str(BLOCKSTACKD_SERVER))
        parser.set('ysi-client', 'port', str(BLOCKSTACKD_PORT))
        parser.set('ysi-client', 'protocol', 'https')
        parser.set('ysi-client', 'metadata', METADATA_DIRNAME)
        parser.set('ysi-client', 'storage_drivers', BLOCKSTACK_DEFAULT_STORAGE_DRIVERS)
        parser.set('ysi-client', 'storage_drivers_required_write', BLOCKSTACK_REQUIRED_STORAGE_DRIVERS_WRITE)
        parser.set('ysi-client', 'api_endpoint_port', str(DEFAULT_API_PORT))
        parser.set('ysi-client', 'api_endpoint_host', DEFAULT_API_HOST)
        parser.set('ysi-client', 'api_endpoint_bind', DEFAULT_API_HOST)
        parser.set('ysi-client', 'queue_path', str(DEFAULT_QUEUE_PATH))
        parser.set('ysi-client', 'poll_interval', str(DEFAULT_POLL_INTERVAL))
        parser.set('ysi-client', 'blockchain_reader', DEFAULT_BLOCKCHAIN_READER)
        parser.set('ysi-client', 'blockchain_writer', DEFAULT_BLOCKCHAIN_WRITER)
        parser.set('ysi-client', 'anonymous_statistics', 'True')
        parser.set('ysi-client', 'client_version', VERSION)

        api_pass = os.urandom(32)
        parser.set('ysi-client', 'api_password', hexlify(api_pass))
    ...
    if ret['ysi-client']['server'] == 'node.ysi.org':
        ysid_port_default = 6263
    else:
        ysid_port_default = 6264

    changed_fields_014_4_3 = {
        'ysi-client': {
            'port' : (str(6264), str(ysid_port_default))
        }
    }

    # should only default to https if we also are pointed at node.ysi.org
    if ret['ysi-client']['server'] == 'node.ysi.org':
            ...
            for changed_field_name in changed_field_set[sec]:
                ...
                if isinstance(changed_field_value, tuple):
                    ...
                    if old_value == prior_default and old_value != new_default:
                        ...
                        # don't go overboard
                        if not (sec == 'ysi-client' and changed_field_name == 'client_version'):
    ...
    if 'ysi-client' in ret:
        ret['ysi-client']['port'] = int(ret['ysi-client']['port'])
...
def get_config(path=CONFIG_PATH, interactive=False):
    ...
    * keep only the ysi-client and bitcoin fields
    ...
    # flatten
    ysi_opts = opts['ysi-client']
    bitcoin_opts = opts['bitcoind']

    bitcoin_opts = opt_restore('bitcoind_', bitcoin_opts)
    ysi_opts.update(bitcoin_opts)

    # pass along the config path and dir, and statistics info
    ysi_opts['path'] = path
    ysi_opts['dir'] = os.path.dirname(path)
    ysi_opts['uuid'] = opts['uuid']
    ysi_opts['client_version'] = ysi_opts.get('client_version', '')
    if 'anonymous_statistics' not in ysi_opts:
        # not disabled
        ysi_opts['anonymous_statistics'] = True
...
def setup_config(config_path=CONFIG_PATH, interactive=False):
    ...
    * back up the old config file if we changed anything during migration.
    ...
    backup_path = None
    conf_version = conf['ysi-client'].get('client_version', '')
