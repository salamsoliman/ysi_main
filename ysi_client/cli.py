...
from ysi_client import config
from ysi_client.client import session
from ysi_client.constants import WALLET_FILENAME, set_secret, serialize_secrets, write_secrets, load_secrets, CONFIG_PATH
from ysi_client.config import CONFIG_PATH, VERSION, client_uuid_path, get_or_set_uuid
from ysi_client.method_parser import parse_methods, build_method_subparsers
...
def get_cli_methods():
    ...
    """
    import ysi_client.actions as builtin_methods
...
def run_cli(argv=None, config_path=CONFIG_PATH):
    ...
    if os.path.exists(wallet_path):
        ...
        if res['migrate'] or res['format'] != 'current':
            if len(sys.argv) <= 1 or sys.argv[1] != 'setup_wallet':
                exit_with_error("Wallet is in legacy format.  Please unlock and migrate it with `ysi setup_wallet`.")
    ...
    ysi_server, ysi_port = conf['ysi-client']['server'], conf['ysi-client']['port']

    # initialize ysi connection
    session(
        server_host=ysi_server,
        server_port=ysi_port, set_global=True
    )
