...
VENV_DIR="$(pwd)/ysi-$BLOCKSTACK_VERSION"
LOGPATH="/tmp/ysi_${BLOCKSTACK_VERSION}_get_started.log"
...
pip install ysi 2>&1 | tee -a "$LOGPATH" || exit_with_error "Failed to install ysi.  Logfile in $LOGPATH"

# test ysi 
test -x "$(which ysi)" || exit_with_error "Failed to find installed ysi program.  Logfile in $LOGPATH"

ysi --version 2>/dev/null | grep "$BLOCKSTACK_VERSION" >/dev/null || exit_with_error "Could not find ysi $BLOCKSTACK_VERSION.  Logfile in $LOGPATH."

logecho "Blockstack $BLOCKSTACK_VERSION installed to $VENV_DIR."
echo ""
echo "Success!  Activate virtualenv with 'source $VENV_DIR/bin/activate' to run 'ysi'"
echo ""
